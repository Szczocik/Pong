...
func _build_def_pose():
	pass
	#var larml = larm[0].x - larm[2].x
	#var larm2 = Rect2(Vector2(larm[2].x, larm[2].y), )
...
func _draw():
	if _step == 0:
		...
		if linew != 0:
			...
			_draw_curve(lleg[0], lleg[1], lleg[2], lcolor, linew, antialiasing)
			_draw_curve(rleg[0], rleg[1], rleg[2], rcolor, linew, antialiasing)
			draw_line(larm[2], lhandp, lcolor, linew, antialiasing)
			draw_line(rarm[2], rhandp, rcolor, linew, antialiasing)
			draw_line(lleg[2], lfootp, lcolor, linew, antialiasing)
			draw_line(rleg[2], rfootp, rcolor, linew, antialiasing)
